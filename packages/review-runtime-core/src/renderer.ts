...
export function createRenderer(rendererOptions: any) {
    ...
    const setupRenderEffect = (instance, container) => {
        ...
        instance.update = effect(
            function componentEffect() {
                //每个组件都有一个effect，vue3是组件级更新，数据变化会重新执行对应的effect
    ...
    const patchKeyedChildren = (c1, c2, container) => {
        ...
        if (i > e1 && i <= e2) {
            ...
            for (let i = toBePatched - 1; i >= 0; i--) {
                ...
                const anchor =
                    nextIndex + 1 < l2
                        ? isFunction(c2[nextIndex + 1].type)
                            ? c2[nextIndex + 1].component.subTree.el
                            : c2[nextIndex + 1].el
                        : null
    ...
    const patchChildren = (n1, n2, container) => {
        ...
        //新的有三种情况 文本 、数组 、空
        //老的有三种情况 文本、数组、空
        if (shapeFlag & ShapeFlags.NULL_CHILDREN) {
            //新的为空
            if (prevShapeFlag & ShapeFlags.NULL_CHILDREN) {
                //老的为空
                //啥也不做
